...
export const moduleX2LocalX = (moduleX: number) => moduleX * COLUMN_WIDTH + GUTTER_SIZE;
...
export const checkCollision = (
  ...
  const currentRight = currentLeft + (currentModule.coord.w * COLUMN_WIDTH - GUTTER_SIZE);
  ...
  for (const otherModule of otherModules) {
    ...
    const otherRight = otherLeft + (otherModule.coord.w * COLUMN_WIDTH - GUTTER_SIZE);
    ...
    const overlapX =
      currentEffective.left < otherEffective.right && currentEffective.right > otherEffective.left;
    const overlapY =
      currentEffective.top < otherEffective.bottom && currentEffective.bottom > otherEffective.top;
